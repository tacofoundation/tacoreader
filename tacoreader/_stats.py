...
def _extract_stats_and_weights(df: "pd.DataFrame") -> tuple[list[list[list[float]]], list[int]]:
    """
    ...
    for band_idx in range(n_bands):
        for class_idx in range(n_classes):
            weighted_sum = sum(
                ...
                for stats, weight in zip(all_stats, weights)
            )
...
def stats_mean(df: "pd.DataFrame") -> np.ndarray:
    ...
    Computes: Σ(mean_i × n_pixels_i) / Σ(n_pixels_i)
...
def stats_std(df: "pd.DataFrame") -> np.ndarray:
    ...
    Computes pooled std: sqrt(Σ((n_i-1)×std_i² + n_i×(mean_i - μ_global)²) / (Σn_i - 1))
...
def _stats_percentile(df: "pd.DataFrame", percentile_idx: int, percentile_name: str) -> np.ndarray:
    """
