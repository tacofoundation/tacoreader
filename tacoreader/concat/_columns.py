...
def _collect_level_columns(datasets: list["TacoDataset"], all_levels: set[str]) -> dict[str, list[set[str]]]:
    ...
    Reads from raw tables (_table suffix), not computed views.
    This ensures we validate storable columns, not generated ones
    like internal:gdal_vsi which is regenerated by ViewBuilder.

    See GENERATED_COLUMNS in _constants.py for details.
    """
    ...
    for level_key in sorted(all_levels):
        ...
        for ds in datasets:
            ...
            if level_key in available_levels:
                # Read from raw table, not view
                # Views have computed columns (gdal_vsi) not in parquet
