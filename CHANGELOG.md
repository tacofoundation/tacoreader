...
### Fixed
- Auto-detect `.tacocat` inside directories (fixes loading `/path/to/dataset` when `.tacocat` exists)

### Changed
- Refactored path resolution into `TacoPath` class (`_path.py`)
- Simplified `loader.py` by centralizing format detection and loading logic
